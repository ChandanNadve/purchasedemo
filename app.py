...
if main_menu == "🛒Purchase":
    ...
    with tab1:
       ...
       with col2P.expander("⚙️ Show Filters"):
                col1, col2 = st.columns([1, 1])

                selected_year_lpo = col1.multiselect(
                    'Choose the Year',
                    sorted(lpo_data['Year'].unique())
                )
                ...
                selected_mon_lpo = col2.multiselect(
                    'Choose the Month',
                    sorted(lpo_data['Month'].unique())
                )
